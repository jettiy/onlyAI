...
const TOP_NAV: MegaItem[] = [
  { label: '홈', path: '/' },
  {
    ...
    children: [
      ...
      { path: '/explore/korean-bench', label: '한국어 벤치마크', icon: <Languages size={18} /> },
      { path: '/explore/context', label: '컨텍스트 비교', icon: <Maximize2 size={18} /> },
      { path: '/explore/tokenizer', label: '토크나이저', icon: <Type size={18} /> },
...
const MOBILE_DRAWER_SECTIONS: { title?: string; items: { path: string; label: string; icon: ReactNode }[] }[] = [
  ...
  },
  {
    ...
    items: [
      ...
      { path: '/changelog', label: '변경 이력', icon: <GitCompareArrows size={18} /> },
