...
const GUIDES: Record<Exclude<InstallType, ''>, Guide> = {
  ...
  docker: {
    title: 'Docker 설치',
    subtitle: '컨테이너로 격리해 실행합니다 (선택 사항).',
    steps: [
      {
        title: '사전 준비',
        desc: 'Docker Desktop 또는 Docker Engine + Docker Compose v2가 필요합니다.',
        bullets: [
          'Windows/Mac: docker.com에서 Docker Desktop 다운로드',
      ...
      {
        title: '공식 이미지로 실행 (권장)',
        desc: 'GitHub Container Registry에서 공식 프리빌드 이미지를 사용합니다.',
        codeBlocks: [
          { label: '1. 리포지토리 클론', code: 'git clone https://github.com/openclaw/openclaw.git\ncd openclaw' },
          { label: '2. 설정 스크립트 실행 (자동 빌드+온보딩)', code: 'export OPENCLAW_IMAGE="ghcr.io/openclaw/openclaw:latest"\n./scripts/docker/setup.sh' },
        ],
        bullets: [
          'setup.sh가 자동으로 이미지 빌드/다운로드 + 온보딩 + Gateway 시작',
          'API 키 입력, 게이트웨이 토큰 생성, .env 파일 작성을 자동 처리',
        ],
      },
      {
        title: '대시보드 열기',
        desc: '브라우저에서 Control UI를 엽니다.',
        codeBlocks: [
          { label: '접속', code: 'http://127.0.0.1:18789/' },
          { label: 'URL이 기억나지 않으면', code: 'docker compose run --rm openclaw-cli dashboard --no-open' },
        ],
      },
      {
        title: '채널 연동 (선택)',
        desc: 'CLI 컨테이너로 메신저를 연동합니다.',
        codeBlocks: [
          { label: 'WhatsApp (QR)', code: 'docker compose run --rm openclaw-cli channels login' },
          { label: 'Telegram', code: 'docker compose run --rm openclaw-cli channels add --channel telegram --token "<BOT_TOKEN>"' },
          { label: 'Discord', code: 'docker compose run --rm openclaw-cli channels add --channel discord --token "<BOT_TOKEN>"' },
        ],
      },
      {
        title: '컨테이너 관리',
        desc: '자주 쓰는 Docker 명령어입니다.',
        codeBlocks: [
          { label: '로그 확인', code: 'docker compose logs -f openclaw' },
          { label: '서비스 중지', code: 'docker compose down' },
          { label: '최신 버전으로 업데이트', code: 'docker compose pull && docker compose up -d' },
        ],
      },
    ],
  },
