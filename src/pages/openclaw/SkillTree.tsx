...
export default function SkillTree() {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* 설치 안내 */}
      <div className="bg-brand-50 dark:bg-brand-950/30 border border-brand-100 dark:border-brand-900 rounded-xl p-4 text-xs text-brand-700 dark:text-brand-300">
        <span className="font-bold">💡 설치 방법:</span>{' '}
        <code className="bg-brand-100 dark:bg-brand-900/50 px-1.5 py-0.5 rounded font-mono">clawhub install [스킬명]</code>{' '}
        또는 <code className="bg-brand-100 dark:bg-brand-900/50 px-1.5 py-0.5 rounded font-mono">openclaw skill install [스킬명]</code>
      </div>

      {/* 검색 */}
      <div className="relative">
        <input type="text" value={search} onChange={e => setSearch(e.target.value)} placeholder="스킬 검색..."
          className="w-full bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-xl px-4 py-2.5 text-sm text-gray-900 dark:text-white placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-brand-500/30 focus:border-brand-500" />
          ...
          <div key={skill.name} className="bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-800 p-4 hover:shadow-sm transition-shadow">
            <div className="flex items-start justify-between gap-3">
              ...
              <button onClick={() => copyInstall(skill.install, skill.name)}
                className="shrink-0 text-[10px] px-2.5 py-1.5 rounded-lg bg-gray-100 dark:bg-gray-800 text-gray-600 dark:text-gray-400 hover:bg-brand-100 hover:text-brand-600 dark:hover:bg-brand-900/30 dark:hover:text-brand-400 transition-colors font-mono">
