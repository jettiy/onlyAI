...
const RESULTS: Record<string, PromptResult[]> = {
  summary: [
    { model: "GPT-5.4", company: "OpenAI", logoId: "openai", score: 95, response: "삼성전자가 HBM4E 양산을 2026년 하반기부터 시작한다. 이 칩은 엔비디아 블랙웰 울트라에 탑재될 전망이다. 성공 시 AI 반도체 시장 판도 변화가 예상된다.", time: "1.0s", strengths: ["핵심 정보 완벽 추출", "자연스러운 문장"], weaknesses: [] },
    ...
    { model: "DeepSeek V3.2", company: "DeepSeek", logoId: "deepseek", score: 88, response: "삼성전자는 HBM4E 개발 완료. 2026년 하반기 양산. 엔비디아 블랙웰 울트라 탑재 예상. 성공 시 시장 판도 변화.", time: "2.1s", strengths: ["핵심 정보 포함", "가격 대비 우수"], weaknesses: ["문장이 끊김"] },
    { model: "Qwen3 235B", company: "Alibaba", logoId: "alibaba", score: 83, response: "삼성전자가 HBM4E를 개발 완료하고 2026 하반기에 양산한다고 발표했다. 대역폭이 HBM3E 대비 2배 향상되었고, 엔비디아 칩에 탑재될 전망이다.", time: "2.5s", strengths: ["정보 누락 없음"], weaknesses: ["중국어식 표현"] },
    { model: "GLM-5", company: "Zhipu AI", logoId: "zhipu", score: 80, response: "삼성전자 HBM4E 양산 계획 발표. 2026년 하반기부터 시작. 엔비디아 탑재 예상. AI 반도체 시장에 영향 예상.", time: "2.0s", strengths: ["빠른 응답"], weaknesses: ["요약 품질 낮음"] },
  ],
  translate: [
    { model: "GPT-5.4", company: "OpenAI", logoId: "openai", score: 96, response: "트랜스포머 아키텍처는 자연어 처리를 다루는 방식을 근본적으로 바꿔놓았다.", time: "0.8s", strengths: ["완벽한 한국어", "전문 용어 적절"], weaknesses: [] },
    ...
    { model: "DeepSeek V3.2", company: "DeepSeek", logoId: "deepseek", score: 82, response: "트랜스포머 아키텍처는 자연어 처리 방식을 근본적으로 변화시켰다.", time: "1.0s", strengths: ["의미 전달"], weaknesses: ["약간 어색한 표현"] },
  ...
  code: [
    { model: "GPT-5.4", company: "OpenAI", logoId: "openai", score: 97, response: "완전한 TODO 앱 구현. TypeScript 타입 정의, 에러 핸들링, 반응형 UI 모두 포함.", time: "3.2s", strengths: ["완벽한 구현", "타입 안전성", "클린 코드"], weaknesses: ["응답 길이 약간 김"] },
    ...
    { model: "Grok 3", company: "xAI", logoId: "xai", score: 93, response: "기능 구현 완료. localStorage 연동 정상 동작.", time: "3.0s", strengths: ["깔끔한 UI 코드"], weaknesses: ["타입 정의 부족"] },
    { model: "DeepSeek V3.2", company: "DeepSeek", logoId: "deepseek", score: 88, response: "기본 기능 구현. TODO 추가/완료/삭제 가능.", time: "4.5s", strengths: ["논리적 사고 과정 표시"], weaknesses: ["CSS 없음", "영어 주석"] },
    ...
    { model: "GLM-5", company: "Zhipu AI", logoId: "zhipu", score: 85, response: "TODO 앱 기본 구현 완료.", time: "3.8s", strengths: ["동작하는 코드"], weaknesses: ["에러 처리 미흡"] },
  ],
  reasoning: [
    { model: "GPT-5.4", company: "OpenAI", logoId: "openai", score: 98, response: "C가 맨 앞입니다. 조건 분석: A>B, C>A이므로 C>A>B. B는 맨 앞이 아니므로 순서는 C>A>B가 확정됩니다.", time: "1.5s", strengths: ["정확한 추론", "명확한 설명"], weaknesses: [] },
    { model: "DeepSeek V3.2", company: "DeepSeek", logoId: "deepseek", score: 97, response: "C가 맨 앞입니다. 단계별로 분석하면...[추론 과정 생략]...따라서 C>A>B의 순서가 됩니다.", time: "3.0s", strengths: ["상세한 추론 과정", "100% 정확"], weaknesses: ["추론 과정이 긺"] },
    { model: "Claude Opus 4.6", company: "Anthropic", logoId: "anthropic", score: 97, response: "C가 맨 앞에 섭니다. A는 B 앞, C는 A 뒤이므로 C가 가장 뒤처 보이지만, B는 맨 앞이 아니라는 조건에서 최종 순서는 C>A>B입니다.", time: "1.8s", strengths: ["정확", "설명 친절"], weaknesses: [] },
  ...
  creative: [
    { model: "GPT-5.4", company: "OpenAI", logoId: "openai", score: 94, response: "2060년의 서울, 용산구의 한 카페에서 AI 비서 미래가 주문을 받고 있었다. 테이블 앞의 지은은 손목의 바이오칩을 톡하자...", time: "2.5s", strengths: ["생생한 묘사", "자연스러운 한국어", "매력적인 전개"], weaknesses: [] },
    { model: "Claude Sonnet 4.6", company: "Anthropic", logoId: "anthropic", score: 93, response: "2060년 서울의 아침은 AI가 지키고 있었다. 도시의 모든 시스템을 관리하는 서울AI가 한강의 수질을 점검하는 동안...", time: "3.0s", strengths: ["세계관 구축 훌륭", "감성적 글쓰기"], weaknesses: ["약간 긺"] },
    ...
    { model: "Grok 3", company: "xAI", logoId: "xai", score: 88, response: "서울, 2060년. 반려 AI 로봇 지훈과 함께 한강 공원을 걷는 지수. AI와 인간이 함께 춤추는 거리...", time: "1.8s", strengths: ["독특한 시각"], weaknesses: ["SF 요소 부족"] },
    { model: "DeepSeek V3.2", company: "DeepSeek", logoId: "deepseek", score: 78, response: "2060년 서울, AI가 사람들의 일상을 돕는 세상. 아침에 AI 비서가 일정을 알려주고, 자율주행 버스가 출근길을 안내한다...", time: "2.0s", strengths: ["스토리 구조"], weaknesses: ["창의성 부족"] },
  ],
  korean: [
    { model: "GPT-5.4", company: "OpenAI", logoId: "openai", score: 96, response: "1. 놀람: \"뭐래? 진짜?\" 2. 분노: \"뭐래? 뭘 그렇게 말해?\" 3. 호기심: \"뭐래? 자세히 말해봐\" 4. 환영: \"뭐래? 잘했네!\" 5. 혼잣말: \"아이고, 뭐래...\"", time: "1.5s", strengths: ["완벽한 뉘앙스 구분", "자연스러운 예시"], weaknesses: [] },
    ...
    { model: "DeepSeek V3.2", company: "DeepSeek", logoId: "deepseek", score: 75, response: "이 문장은 다양한 뉘앙스를 가집니다. 1. 의문 2. 놀람 3. 비난 4. 호기심 5. 혼잣말. 상황에 따라 의미가 크게 달라집니다.", time: "2.5s", strengths: ["카테고리 분류"], weaknesses: ["한국어 미묘한 뉘앙스 부족"] },
