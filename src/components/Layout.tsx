...
const TOP_NAV: MegaItem[] = [
  { label: '홈', path: '/' },
  {
    ...
    children: [
      ...
      { path: '/pricing', label: '가격표', icon: <DollarSign size={18} /> },
      // 아래 항목들은 '더 보기' 섹션으로 분리
      // { path: '/explore/calculator', label: '가격 계산기' },
      // { path: '/explore/timeline', label: '타임라인' },
      // { path: '/explore/tokenizer', label: '토큰 체험기' },
      // { path: '/explore/context-window', label: '컨텍스트 시각화' },
      // { path: '/explore/korean-bench', label: '언어 성능' },
      // { path: '/explore/prompt-bench', label: '프롬프트 벤치마크' },
      // { path: '/explore/guide', label: '시작 가이드' },
      // { path: '/video/timeline', label: '비디오 AI' },
      // { path: '/image', label: '이미지 AI' },
    ],
  },
...
const MOBILE_DRAWER_SECTIONS: { title?: string; items: { path: string; label: string; icon: ReactNode }[] }[] = [
  ...
  },
  {
    ...
    items: [
      ...
      { path: '/explore/korean-bench', label: '언어 성능', icon: <Globe size={18} /> },
